...
void Follow(float leftlean, float rightlean){
		...
		while(SensorValue[S3] > 40){ //further than 11 cm away
...
void ReverseFollow(float leftlean, float rightlean, int finalValue){
		...
		while(count != (finalValue-1)){
			...
			if(left >= leftlean && right >= rightlean){
				...
				if(Solution[finalValue - value] == 1){
						Bump();
						...
						motor[motorC] = 10;
					 	...
					 	wait1Msec(2000);
				} else if (Solution[finalValue - value] == 3){
						Bump();
						...
						motor[motorC] = -10;
					 	...
					 	wait1Msec(2000);
				} else if (Solution[finalValue - value] == 2){
